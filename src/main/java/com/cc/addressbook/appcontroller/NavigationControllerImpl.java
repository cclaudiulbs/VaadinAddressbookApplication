	...
	public void dispatch(MenuActionType pressedMenuAction) {
		...
		if(pressedMenuAction instanceof AddressboookHorizontalMenu) {
			...
			if(pressedMenuAction == HorizontalMenuBarActions.ADD_CONTACT) {
				...
				final ShowAllContactsPresenter showContactPresenter = new ShowAllContactsPresenter(mainView,
						showAllContactsView);
				...
				final AddressbookMainView.LoginViewPresenter loginViewPresenter = new LoginViewPresenterImpl(mainView,
						loginView);
				new AuthenticatePresenterImpl(loginView);
